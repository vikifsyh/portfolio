...
import Profil from "@/img/ava.jpg";
...
export default function Nav() {
  ...
  return (
    <Navbar
      className="block sm:hidden bg-white shadow-sm"
      ...
      <NavbarContent>
        <NavbarBrand>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-medium text-black flex gap-1 justify-center items-center">
              Viki Flendiansyah
              <svg
                width="18"
                height="18"
      ...
      <NavbarMenu className="bg-white">
        {menuItems.map((item, index) => (
          <NavbarMenuItem key={`${item}-${index}`}>
            <Link
              className="w-1/2 text-black rounded-md text-base px-4 hover:bg-background hover:ease-in duration-300"
              href="#"
              size="lg"
            >
              {item}
